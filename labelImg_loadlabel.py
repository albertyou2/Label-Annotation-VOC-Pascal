...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def loadFile(self, filename=None):
        ...
        if filename and QFile.exists(filename):
            ...
            # albertyou
            print self.usingPascalVocFormat
            print self.defaultSaveDir 
            #self.defaultSaveDir = os.path.dirname(os.path.abspath(__file__))+'/Images/010_label'

            # if self.usingPascalVocFormat is True and self.defaultSaveDir is not None:
            #         basename = os.path.basename(os.path.splitext(self.filename)[0]) + '.xml'
            #         xmlPath = os.path.join(self.defaultSaveDir, basename)
            #         self.loadPascalXMLByFilename(xmlPath)
            #         print xmlPath
            if self.usingPascalVocFormat is True and self.labelDir is not None:
                    basename = os.path.basename(os.path.splitext(self.filename)[0]) + '.xml'
                    xmlPath = os.path.join(self.labelDir, basename)
                    self.loadPascalXMLByFilename(xmlPath)
                    print xmlPath
    ...
    def openDir(self, _value=False):
        ...
        # albertyou 
        self.labelDir = self.dirname+'_label'
